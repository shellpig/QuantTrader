...
3. **AI 技術分析問答** — 以自然語言詢問技術分析問題，Claude AI 自動查詢數據並給出分析
...
| **AI 功能** | `anthropic` | Claude API，Tool Use 模式 |
...
│  ├── RiskManager       個人版簡化風控                          │
│  ├── AIAdvisor         Claude API Tool Use 整合               │
...
│  └── Claude API      AI 分析功能（Anthropic，按 token 計費）   │
└─────────────────────────────────────────────────────────────┘
```
...
│   ├── ai/
│   │   └── advisor.py        Claude API Tool Use 整合
│   └── ui/
...
使用者以**自然語言**提問，系統自動查詢數據、計算指標，由 Claude AI 生成分析報告。
...
### 7.2 Claude API Tool Use 設計

採用 Function Calling（Tool Use）模式，讓 Claude 決定要查哪些資料，再整合結果生成自然語言回應。

#### 定義給 Claude 的工具（Tools）
...
1. 使用者輸入問題
        ↓
2. Claude API（帶入 tools 定義）
   → Claude 決定呼叫哪些工具、哪些參數
        ↓
3. 本機執行工具（查詢 DuckDB / 計算 pandas-ta 指標）
        ↓
4. 將工具回傳結果送回 Claude API
        ↓
5. Claude 生成自然語言分析（附免責聲明）
        ↓
6. 顯示於 Streamlit UI
```
...
`pandas-ta` 的指標命名與台股慣用名不一致，`IndicatorEngine` 需維護對照表，讓 Claude Tool Use 可以使用台股慣用名：
...
# 編輯 .env 填入 ANTHROPIC_API_KEY、FINMIND_TOKEN
...
```
ANTHROPIC_API_KEY=sk-ant-...      # Claude API Key（AI 問答功能必要）
...
#### 4-A　Claude API Tool Use 基礎（Day 1-2）

**建置內容：** 三個工具定義（`get_price_data` / `calculate_indicators` / `get_support_resistance`）、工具呼叫解析

**驗收指標：**
- 輸入「2330 的 RSI」→ Claude 呼叫正確工具（工具名稱記錄驗證）
- 工具回傳 mock 資料 → Claude 能整合並生成回應
- 每次回應底部包含免責聲明文字（字串包含檢查）

#### 4-B　技術指標工具實作（Day 2-3）

**建置內容：** pandas-ta 封裝，對應 Claude Tool Use 定義的介面
...
- 完整問答流程 < 30 秒（使用者體驗基準；含 Claude API Tool Use 多輪往返、工具執行、最終 LLM 回應；不含首次冷啟動）
...
| **Claude API（AI 問答）** | 約 $1-5 USD/月 | 依問答頻率；claude-haiku 模型最省費 |
...
- 若 Claude API 費用過高 → 增加 prompt caching 優化、或限制每日問答次數
...
3. **AI 局限性：** Claude AI 的分析基於提供的數值資料進行統計描述，AI 無法預測市場走向。分析中使用的技術術語（如「看漲」、「支撐」、「超買」）均為技術分析的慣用描述，非承諾或預測。
...
*個人研究工具，技術棧：Python 3.12 + DuckDB + Claude API + Streamlit。*
