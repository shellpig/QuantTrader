...
| 11-D | Goodinfo 股利政策 fallback | 事件行事曆不再用去年除息日 / 股利硬推預估；改用 Goodinfo 股利政策頁作參考 fallback | 否，僅每日 cache |
...
| `GET /api/analysis/p11/event-calendar?symbol={s}` | 11-C / 11-D | `{ next_ex_dividend, last_ex_dividend, dividend_policy_fallback, next_shareholder_meeting, last_shareholder_meeting }` |
...
#### 11-D：Goodinfo 股利政策 fallback

11-D 取代原先未定義的「散戶多空比」placeholder，改為補強事件行事曆的「即將除息」顯示。原因：目前若本機 `dividends.parquet` 沒有未來正式除息資料，系統會用上一筆除息日期 + 1 年與上一筆現金股利產生 `[預估]`，例如 `3293` 顯示 `2026-07-17 現金股利 29.00 [預估] 倒數 60 天`。這種預估值常不準，應停止顯示成準事件。

11-D 不新增正式資料層，不寫入 `dividends.parquet`，不進 `data_meta`，不輸出 CSV。Goodinfo 只作事件行事曆的即時參考 fallback，並加每日 cache。

Goodinfo URL：

```text
https://goodinfo.tw/tw/StockDividendPolicy.asp?STOCK_ID={symbol}
```

資料擷取規則：

1. 若 `load_dividends` 抓到未來正式除息資料，直接顯示正式除息事件：
   - `next_ex_dividend.is_estimated = false`
   - 顯示日期、倒數天數、現金股利。
...
- 不做散戶多空比演算法；原 11-D placeholder 已改為 Goodinfo 股利政策 fallback。
